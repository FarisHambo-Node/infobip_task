...
try:
    args = getResolvedOptions(
        sys.argv,
        ["JOB_NAME", "S3_BUCKET", "ARTIFACTS_BUCKET", "DB_HOST", "DB_NAME", "DB_USER", "DB_PASSWORD"],
    )
...
def load_data_from_s3_to_rds():
    ...
    try:
        ...
        logger.info(f"Converted to Pandas - Customers: {len(customers_pandas)}, Traffic: {len(traffic_pandas)}, Channels: {len(channels_pandas)}")
    except Exception as e:
    ...
    try:
        ...
        for _, row in customers_pandas.iterrows():
            ...
            """, (row['account_id'], row['customer_name'], row['segment'], 
                  row['industry'], row['country'], row['created_date']))
        ...
        for _, row in traffic_pandas.iterrows():
            ...
            cursor.execute("SELECT customer_id FROM customers WHERE account_id = %s", (row['account_id'],))
            customer_result = cursor.fetchone()
            if customer_result:
                customer_id = customer_result[0]
            else:
                logger.warning(f"Customer with account_id {row['account_id']} not found, skipping traffic record")
                continue
            
            # Get channel_id from channels table based on channel_id from CSV
            cursor.execute("SELECT channel_id FROM channels WHERE channel_id = %s", (row['channel_id'],))
            channel_result = cursor.fetchone()
            if channel_result:
                channel_id = channel_result[0]
            else:
                logger.warning(f"Channel with channel_id {row['channel_id']} not found, skipping traffic record")
                continue
            ...
            """, (row['send_date'], row['account_id'], customer_id, channel_id, 
                  row['interactions_count'], row['revenue_eur']))
...
def setup_base_schema(cursor, artifacts_bucket):
    ...
    execute_sql_file(cursor, artifacts_bucket, "database/schema.sql", return_results=False)
...
def setup_analytics_schema(cursor, artifacts_bucket):
    ...
    execute_sql_file(cursor, artifacts_bucket, "sql/analytics_schema.sql", return_results=False)
...
def execute_business_queries():
    ...
    try:
        ...
        results1 = execute_sql_file(cursor, artifacts_bucket, "sql/question_1_industry_exposure.sql")
        ...
        results2 = execute_sql_file(cursor, artifacts_bucket, "sql/question_2_segment_analysis.sql")
        ...
        for row in results2:
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s);
            """, (row[0], row[1], float(row[2]), float(row[3]) if row[3] else 0, float(row[4]) if row[4] else 0))
        ...
        results3 = execute_sql_file(cursor, artifacts_bucket, "sql/question_3_recent_customers.sql")
        ...
        results4 = execute_sql_file(cursor, artifacts_bucket, "sql/question_4_top_customers.sql")
        ...
        for i, row in enumerate(results4, 1):
            ...
            """, (i, row[0], row[1], row[2], row[3], row[4], float(row[5]), row[6], 
                  float(row[7]), row[8], row[9]))
        ...
        results5 = execute_sql_file(cursor, artifacts_bucket, "sql/question_5_monthly_active_customers.sql")
        ...
        for row in results5:
            ...
            """, (row[0], row[1], row[2], row[3], float(row[4]), row[5], row[6], row[7], float(row[9])))
...
def main():
    ...
    try:
        ...
        logger.info(f"Business analytics job completed successfully at {datetime.now()}")
